...
export default function Candidates() {
  ...
  const { data: rows = [], isLoading, error } = useQuery({
    ...
    queryFn: () => listModuleRows({ table: "candidates", module: "candidates", currentUser: authUser }),
    enabled: !!authUser?.id,
  });

  const allowedKeys = useMemo(() => formColumns.map((f) => f.field_key), [formColumns]);
  const editable = canEdit(user, "candidates");

  const createMutation = useMutation({
    mutationFn: (payload) => createAuditedModuleRow({ table: "candidates", module: "candidates", payload, currentUser: authUser, allowedKeys }),
    onSuccess: async () => {
      toast.success("Candidate created");
      await queryClient.invalidateQueries({ queryKey: ["candidates", authUser?.id] });
      setOpen(false); setEditingId(null); setForm({});
    },
    onError: (err) => toast.error(err?.message || "Failed to create candidate"),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateAuditedModuleRow({ table: "candidates", module: "candidates", id, payload, currentUser: authUser, allowedKeys }),
    onSuccess: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => deleteAuditedModuleRow({ table: "candidates", module: "candidates", id, currentUser: authUser }),
    onSuccess: async () => {
      toast.success("Candidate deleted");
      await queryClient.invalidateQueries({ queryKey: ["candidates", authUser?.id] });
    },
    onError: (err) => toast.error(err?.message || "Failed to delete candidate"),
  });
  ...
  return (
    <AppLayout heroRight={editable ? <Button onClick={handleOpenAdd} className="rounded-xl"><Plus className="mr-2 h-4 w-4" />Add Candidate</Button> : null}>
      <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {userLoading || isLoading ? <div className="p-6 text-sm text-slate-500">Loading candidates...</div> : error ? <div className="p-6 text-sm text-red-600">Failed to load candidates.</div> : rows.length === 0 ? <div className="p-6 text-sm text-slate-500">No candidates found.</div> : (
          <Table>
      ...
      <RecordFormModal open={open} onOpenChange={setOpen} title={editingId ? "Edit Candidate" : "Add Candidate"}>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <AttachmentUploader />
          <div className="flex justify-end gap-3 pt-2"><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit">{editingId ? "Update Candidate" : "Save Candidate"}</Button></div>
        </form>
      </RecordFormModal>

      <Dialog open={viewOpen} onOpenChange={setViewOpen}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Candidate Details</DialogTitle></DialogHeader>{selectedRow ? <div className="grid gap-4 md:grid-cols-2">{tableColumns.map((c) => <div key={c.field_key} className="rounded-xl border border-slate-200 p-4"><div className="text-xs font-semibold uppercase tracking-wide text-slate-400">{c.field_label}</div><div className="mt-2 text-sm text-slate-900">{String(selectedRow?.[c.field_key] ?? "—")}</div></div>)}</div> : null}</DialogContent></Dialog>
    </AppLayout>
  );
}
