...
export default function Timesheets() {
  ...
  const { data: rows = [], isLoading, error } = useQuery({
    ...
    queryFn: () => listWorkforceRows({ table: "timesheets", module: "timesheets", currentUser: authUser, employeeColumn: "employee_id" }),
    enabled: !!authUser?.id,
  });

  const { data: userOptions = [] } = useQuery({
    queryKey: ["organization-user-options", authUser?.id],
    queryFn: () => listOrganizationUserOptions(authUser),
    enabled: !!authUser?.id,
  });
  ...
  const createMutation = useMutation({
    mutationFn: (payload) => createWorkforceRow({ table: "timesheets", module: "timesheets", payload: { ...payload, status: payload.status || "draft", employee_id: payload.employee_id || authUser.id }, currentUser: authUser, allowedKeys, employeeColumn: "employee_id" }),
    onSuccess: async () => { toast.success("Timesheet created"); await queryClient.invalidateQueries({ queryKey: ["timesheets", authUser?.id] }); setForm({}); setEditingId(null); setOpen(false); },
    onError: (error) => toast.error(error.message || "Failed to create timesheet"),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateWorkforceRow({ table: "timesheets", module: "timesheets", id, payload, currentUser: authUser, allowedKeys, employeeColumn: "employee_id" }),
    onSuccess: async () => { toast.success("Timesheet updated"); await queryClient.invalidateQueries({ queryKey: ["timesheets", authUser?.id] }); setForm({}); setEditingId(null); setOpen(false); },
    onError: (error) => toast.error(error.message || "Failed to update timesheet"),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => deleteWorkforceRow({ table: "timesheets", module: "timesheets", id, currentUser: authUser, employeeColumn: "employee_id" }),
    onSuccess: async () => { toast.success("Timesheet deleted"); await queryClient.invalidateQueries({ queryKey: ["timesheets", authUser?.id] }); },
