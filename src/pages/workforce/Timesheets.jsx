...
async function listTimesheets(currentUser) {
...
export default function Timesheets() {
  ...
  const { data: timesheets = [], isLoading, error } = useQuery({
    ...
    queryFn: () => listTimesheets(authUser),
    enabled: !!authUser?.id,
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["timesheets", authUser?.id] });
      setForm({});
      setEditingId(null);
      setOpen(false);
    },
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage workforce timesheets.</p>
        </div>
        <button
      ...
      <div className="rounded-2xl border overflow-hidden bg-card">
        {isLoading ? (
          ...
          <div className="p-6 text-muted-foreground">No timesheets yet.</div>
        ) : (
