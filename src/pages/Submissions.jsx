...
async function listSubmissions(currentUser) {
...
export default function Submissions() {
  ...
  const { data: submissions = [], isLoading, error } = useQuery({
    queryKey: ["submissions", authUser?.id],
    queryFn: () => listSubmissions(authUser),
    enabled: !!authUser?.id,
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["submissions", authUser?.id] });
      setForm({});
      setEditingId(null);
      setOpen(false);
    },
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage shared submission records for your organization.
          </p>
        </div>
        <button
