...
const HIDDEN_DUPLICATE_FIELDS = ["candidate_id","candidate_name","job_id","job_title","client_id","client_name"];
...
export default function Placements() {
  ...
  const { tableColumns = [], formColumns = [] } = useModuleColumns("placements");

  const { data: rows = [], isLoading, error } = useQuery({ queryKey: ["placements", authUser?.id], queryFn: () => listModuleRows({ table: "placements", module: "placements", currentUser: authUser }), enabled: !!authUser?.id });
  ...
  const editable = canEdit(user, "placements");

  const createMutation = useMutation({ mutationFn: (payload) => createAuditedModuleRow({ table: "placements", module: "placements", payload, currentUser: authUser, allowedKeys }), onSuccess: async () => { toast.success("Placement created"); await queryClient.invalidateQueries({ queryKey: ["placements", authUser?.id] }); setOpen(false); setEditingId(null); setForm({}); }, onError: (err) => toast.error(err?.message || "Failed to create placement") });
  const updateMutation = useMutation({ mutationFn: ({ id, payload }) => updateAuditedModuleRow({ table: "placements", module: "placements", id, payload, currentUser: authUser, allowedKeys }), onSuccess: async () => { toast.success("Placement updated"); await queryClient.invalidateQueries({ queryKey: ["placements", authUser?.id] }); setOpen(false); setEditingId(null); setForm({}); }, onError: (err) => toast.error(err?.message || "Failed to update placement") });
  const deleteMutation = useMutation({ mutationFn: (id) => deleteAuditedModuleRow({ table: "placements", module: "placements", id, currentUser: authUser }), onSuccess: async () => { toast.success("Placement deleted"); await queryClient.invalidateQueries({ queryKey: ["placements", authUser?.id] }); }, onError: (err) => toast.error(err?.message || "Failed to delete placement") });
  ...
  return (
    <AppLayout heroRight={editable ? <Button onClick={handleOpenAdd} className="rounded-xl"><Plus className="mr-2 h-4 w-4" />Add Placement</Button> : null}>
      <div className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {userLoading || isLoading ? <div className="p-6 text-sm text-slate-500">Loading placements...</div> : error ? <div className="p-6 text-sm text-red-600">Failed to load placements.</div> : rows.length === 0 ? <div className="p-6 text-sm text-slate-500">No placements found.</div> : (
      ...
      <RecordFormModal open={open} onOpenChange={setOpen} title={editingId ? "Edit Placement" : "Add Placement"}><form onSubmit={handleSubmit} className="space-y-6"><div className="grid gap-4 md:grid-cols-2"><SearchableEntitySelect label="Candidate" value={form.candidate_id} onChange={(value, opt) => setForm((prev) => ({ ...prev, candidate_id: value, candidate_name: opt?.label || "" }))} options={candidateOptions} /><SearchableEntitySelect label="Job" value={form.job_id} onChange={(value, opt) => setForm((prev) => ({ ...prev, job_id: value, job_title: opt?.label || "" }))} options={jobOptions} /><SearchableEntitySelect label="Client" value={form.client_id} onChange={(value, opt) => setForm((prev) => ({ ...prev, client_id: value, client_name: opt?.label || "" }))} options={clientOptions} />{visibleFormColumns.map((field) => <div key={field.field_key} className={field.field_type === "textarea" ? "md:col-span-2" : ""}><label className="mb-1 block text-sm">{field.field_label}{field.required ? <span className="ml-1 text-red-500">*</span> : null}</label><DynamicField field={field} value={form[field.field_key]} onChange={(key, value) => setForm((prev) => ({ ...prev, [key]: value }))} /></div>)}</div><div className="flex justify-end gap-3 pt-2"><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button type="submit">{editingId ? "Update Placement" : "Save Placement"}</Button></div></form></RecordFormModal>
      <Dialog open={viewOpen} onOpenChange={setViewOpen}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Placement Details</DialogTitle></DialogHeader>{selectedRow ? <div className="grid gap-4 md:grid-cols-2">{tableColumns.map((c) => <div key={c.field_key} className="rounded-xl border border-slate-200 p-4"><div className="text-xs font-semibold uppercase tracking-wide text-slate-400">{c.field_label}</div><div className="mt-2 text-sm text-slate-900">{String(selectedRow?.[c.field_key] ?? "—")}</div></div>)}</div> : null}</DialogContent></Dialog>
    </AppLayout>
  );
}
